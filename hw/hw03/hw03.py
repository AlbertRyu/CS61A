...
def missing_digits(n):
    ...
    def helper(count,n,current_digit):
        #print(f'now count {count}, n is {n}, and current_dig is {current_digit}')
        last_digit = n % 10
        if n // 10 == 0 and (last_digit == current_digit-1 or last_digit == current_digit):
            #print(f'now return! {count}')
            return count
        else:
            if last_digit == current_digit:
                #print('same')
                return helper(count,n//10,current_digit)
            elif last_digit == current_digit - 1:
                #print('go on')
                return helper(count,n//10,current_digit-1)
            else:
                #print('count 1')
                return helper(count+1,n,current_digit-1)
