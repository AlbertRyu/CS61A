...
def choose(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    available = [para for para in paragraphs if select(para)]
    if k < len(available): 
        return available[k]
    else: return ''
    # END PROBLEM 1
...
def about(topic):
    ...
    "*** YOUR CODE HERE ***"
    def func(sentence):
        t = sentence.translate(str.maketrans('','',string.punctuation)).split()
        for i in topic:
            if i in [w.lower() for w in t]:
                return True
        else: return False
    return func
    # END PROBLEM 2
...
def accuracy(typed, reference):
    ...
    for i,j in zip(typed_words,reference_words):
        if i==j:
            count += 1  
    return 0.0 if len(typed_words)==0 else count/len(typed_words) * 100     
    # END PROBLEM 3
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    if user_word in valid_words or min([diff_function(user_word,x,limit) for x in valid_words])>limit:
        ...
        return min(valid_words,key=func)
    # END PROBLEM 5
